...
module.exports = ->
  (file, options, run = no) ->
    cs = fs.readFileSync "./examples/#{file}.js", 'utf-8'
    js = desugar.transform cs, options # This is just the initial JavaScript output; it is replaced by dynamic compilation on changes of the CoffeeScript pane.
    render = _.template fs.readFileSync('./site/code.html', 'utf-8')
    include = (file) -> fs.readFileSync("./site/#{file}", 'utf-8')
